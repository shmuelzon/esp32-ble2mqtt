    ...
    int rssi, broadcaster_meta_data_cb_t cb, void *ctx)
{
    ...
    if (frame_ctrl & 0x08)
    {
        ...
        if (version < 4)
        {
            ESP_LOGW(TAG, "Legacy MiBeacon encryption not supported, skipping %s", _mactoa(mijia_header->mac));
            return;
        }
        ...
        if (!key_str)
        {
            ESP_LOGE(TAG, "MiBeacon decryption key not found for %s", _mactoa(mijia_header->mac));
            return;
        }
        if (strlen(key_str) != sizeof(key) * 2)
        {
            ESP_LOGE(TAG, "MiBeacon decryption key for %s has the wrong length", _mactoa(mijia_header->mac));
            return;
        }
