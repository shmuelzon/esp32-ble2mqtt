    ...
    ble_uuid_t characteristic_uuid, const uint8_t *value, size_t value_len)
{
    ...
    ble_operation_type_t type = BLE_OPERATION_TYPE_WRITE;
    if (characteristic->properties & CHAR_PROP_WRITE_NR)
        type = BLE_OPERATION_TYPE_WRITE_NR;

    ble_operation_enqueue(&operation_queue, type, device,
